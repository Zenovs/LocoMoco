...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Anzeige: letzte 12 Monate. Der Über-Budget-Zeitpunkt wird trotzdem über
    // ALLE Einträge berechnet — via Startwert (Lebenszeit-Gesamt − Fenstersumme).
    ...
    // Gebuchte Stunden pro Aufgabe (Leistung) aus den Aktivitäten — zuverlässig
    // (das Task-Budget aus MOCO ist in CHF, nicht in Stunden, daher hier weggelassen).
    const taskHours = new Map<string, number>();
    for (const a of activities) {
      const name = a.task?.name ?? "—";
      taskHours.set(name, (taskHours.get(name) ?? 0) + a.hours);
    }
    const tasks = [...taskHours.entries()]
      .map(([name, hours]) => ({ name, actual: Math.round(hours * 10) / 10 }))
