  ...
  return (
    <section className="card">
      ...
      <p style={{ fontSize: 12.5, color: "var(--plum-soft)", fontWeight: 600, marginBottom: 20 }}>
        Projekte mit dem höchsten Anteil interner Stunden
      </p>
          ...
          {projects.map((p, i) => (
            <div key={p.projectId}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 7 }}>
                ...
                <span style={{ fontFamily: "Fredoka, sans-serif", fontWeight: 700, fontSize: 14, color: "var(--hotpink)", whiteSpace: "nowrap", marginLeft: 8 }}>
                  {p.nonBillablePct}%
                </span>
              </div>
              <div className="bar">
                <i style={{ width: `${p.nonBillablePct}%` }} />
              </div>
            </div>
          ))}
        </div>
